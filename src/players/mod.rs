use crate::players::{uai::UAIEngine, uci::UCIEngine, ugi::UGIEngine};
...
#[must_use]
pub fn create(name: &str, path: &str, proto: Protocol) -> Box<dyn Player> {
    match proto {
        Protocol::UGI => Box::new(UGIEngine::new(name, path)),
        Protocol::UAI => Box::new(UAIEngine::new(name, path)),
        Protocol::UCI => Box::new(UCIEngine::new(name, path)),
    }
}
